...
let generateRandomPassword = function (randomNum) {
  ...
  //This part is to make sure that the password contain at least one number. If not, replace a character with a number.
  if (/\d/.test(passwords)) {
    return passwords
  } else {
...
// I really have to name the functions better, generatePassword() and generatePasswords() are very confusing...
...
// If the password is not empty then copy it to the clipboard
